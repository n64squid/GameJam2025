...
#define GAME_HUD_ROBOT_PARTS_PADDING_X (GAME_HUD_PADDING_MID)
#define GAME_HUD_ROBOT_PARTS_PADDING_Y (GAME_HUD_PADDING_BIG + 1)
...
void game_hud_draw (void) {
	...
	for (uint8_t i=0; i<ROBOT_PARTS_COUNT; i++) {
		...
		rdpq_text_printf(NULL, 1,
			GAME_HUD_ROBOT_PARTS_X + GAME_HUD_PADDING_MID,
			GAME_HUD_ROBOT_PARTS_Y + i * (GAME_HUD_ROBOT_PARTS_HEIGHT + GAME_HUD_PADDING_SML) + GAME_HUD_PADDING_BIG + 1,
			"%s",
			robot.parts[i].name
		);
